...
class ESPFirmwareImage:
    ...
    # NodeMCU OTA magic values
    OTA_IMAGE_MAGIC = 0xef50a831
    BOOT_STATUS_MAGIC = 0x138a05fe
    OTA_HEADER_MAGIC = 0x4d4a
    ...
    def save_ota(self, filename, initially_valid, max_tests):
        ...
        for i in range(0, max_tests):
          boot_bits = (boot_bits << 1) + 1
        f.write (struct.pack('<HBBI', ESPFirmwareImage.OTA_HEADER_MAGIC, boot_bits, flags | len(self.segments), self.entrypoint))
        for (offset, size, data) in self.segments:
            f.write(struct.pack('<II', offset, size))
            f.write(data)
        irom = e.load_section(".irom0.text")
        off = e.get_symbol_addr("_irom0_text_start") - 0x40200000 - 0x1000
        assert off >= 0
        f.seek(off)
        f.write(irom)
        f.close()
...
if __name__ == '__main__':
    ...
    parser_elf2ota = subparsers.add_parser(
      'elf2ota',
      help = 'Create an OTA image from an ELF file')
    parser_elf2ota.add_argument('input', help = 'Input ELF file')
    parser_elf2ota.add_argument('--output', '-o', help = 'Output filename', type = str)
    parser_elf2ota.add_argument('--max_tests', '-t', help = 'Number of test boot attempts', default = 3, type = int)
    parser_elf2ota.add_argument('--initially_valid', '-v', help = 'Mark image as initially valid (for direct flashing only!)', action = 'store_true', default = False)
    ...
    if args.operation == 'image_info':
        ...
        image.save_ota(args.output, args.initially_valid, args.max_tests)
